...
(() => {
  const stylesheet = `
    ...
    .toolbar button {
      appearance: none;
      border: 1px solid rgba(233, 180, 85, 0.24);
      border-radius: 99px;
    ...
    .toolbar button:hover { border-color: #e9b455; color: #f2d789; }
    ...
    .note {
      ...
      bottom: 16px;
      max-width: 60%;
      font: 400 11.5px/1.5 "JetBrains Mono", ui-monospace, monospace;
      color: rgba(244, 238, 218, 0.62);
  ...
  class ThreeDStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._boot().catch((err) => {
        ...
        this._err.textContent =
          ...
          'It needs WebGL, and the import map in <head> must resolve three.js ' +
          '(this page maps it to campus/vendor/three).\n\n' +
    ...
    async _boot() {
      ...
      // The host page may prefer to render its own controls (and call
      // download() for exports) — drop the built-in chrome when asked.
      // Read here, not in the constructor: attributes are not parsed yet
      // when the parser upgrades the element.
    ...
    /** Export the current object as 'obj' (OBJ + MTL) or 'glb'. Public so a
     *  host page using hide-chrome can wire its own download buttons.
     *  Resolves once the files are handed to the browser; rejects if the
     *  export itself fails. */
    download(format) {
      return this._runExport(format === 'obj' ? 'obj' : 'glb');
    }
