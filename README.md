...
Contact); on phones the pill scrolls sideways with a fade on the edge that still has more.
...
- `trading/` — the autonomous paper-trading desk: bots, routines, P&L, risk engine, postmortem.
  Every number is a snapshot of 17 Jul 2026 and the page says so.
...
- `fow/` — FoW, the Future of Work demo: sign in as one of seven personas, each with its own
  paradigm, MCP-connected tiles, guided tours and an animated agent crew. One 800 KB file.
- `onesheet/` — a spreadsheet engine in a single offline HTML file. `=AI()` runs in mock mode
  by default; a real key is kept in `sessionStorage` for the tab only.
...
- `.github/workflows/deploy-pages.yml` — auto-deploys to GitHub Pages on every push to `main`,
  leaving out the unused film frames and the font build script.
